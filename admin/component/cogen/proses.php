...
$compo_browse = trim(strtolower($_POST['compo_browse']));
	...
	if (file_exists($dirpath)){
		...
		mkdir($dirpath, 755);
		copy("../../component/index.html", "".$dirpath."/index.html");
		echo "<li class='list-group-item'>- DELETE OLD DIRECTORY `".$compo_name."`</li>";
		echo "<li class='list-group-item'>- MAKE NEW DIRECTORY `".$compo_name."`</li>";
	}else{
		mkdir($dirpath, 755);
	...
	echo "<li class='list-group-item'>- CREATE FILE ".$compo_name.".php, datatable.php, javascript.js and proses.php INTO FOLDER `".$compo_name."`</li>";

	$dumpingpoint = <<<EOS
<?php
//Author  	= 'CompoGen';
//Contact 	= 'mailto:info@wisasi.com';
session_start();
if (empty(\$_SESSION['namauser']) AND empty(\$_SESSION['passuser'])){
	header('location:404.php');
}else{
\$aksi="component/{$compo_name}/proses.php";
?>
...
EOS;
                ...
                $dumpingpoint4 = <<<EOS
                    <div class="form-group">
                        <label>{$uifields} <span class="text-danger">*</span></label>
                        <textarea class="form-control" id="w-wysiwyg" name="{$ifields['compo_field_name']}" style="height:500px;" required></textarea>
                    </div>\n
EOS;
            } else {
                if ($compo_browse == $ifields['compo_field_name']) {
...
<?php
	...
	\$table = new WisasiTable('{$compo_table}');
...
EOS;
                ...
                $dumpingpoint6 = <<<EOS
                    <div class="form-group">
                        ...
                        <label>{$uifields} <span class="text-danger">*</span></label>
                        <textarea class="form-control" id="w-wysiwyg" name="{$ifields['compo_field_name']}" style="height:500px;" required><?=\$dutf;?></textarea>
                    </div>\n
EOS;
            } else {
                if ($compo_browse == $ifields['compo_field_name']) {
...
EOS;
    ...
    while ( \$aRow = mysql_fetch_array( \$rResult ) )
    {
        ...
        for ( \$i=1 ; \$i<count(\$aColumns) ; \$i++ )
        {
			...
			\$strlink = preg_replace("/\/admin\/component\/{$compo_name}\/(datatable\.php$)/","",\$str);
			{$tbldelete}
			{$checkdata}
			{$rowcheckdata}
...
oTable = $('.dTableAjax').dataTable({
	"sAjaxSource": "component/{$compo_name}/datatable.php",
...
if (\$mod=='{$compo_name}' AND \$act=='delete'){
	\$id = \$val->validasi(\$_POST['id'],'sql');
	\$tabledel = new WisasiTable('{$compo_table}');
...
EOS;
	...
	$regcomponent = $compo_name;
	$tablereg = new WisasiTable('component');
