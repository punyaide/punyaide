oTable = $('.dTableAjax').dataTable({
	"sAjaxSource": "component/pages/datatable.php",
...
$('.btn-del-image').click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "component/pages/proses.php",
        data: dataString,
        cache: false,
...
$('#tiny-text').click(function (e) {
	e.stopPropagation();
	tinymce.EditorManager.execCommand('mceRemoveEditor',true, 'w-wysiwyg');
});

$('#tiny-visual').click(function (e) {
	e.stopPropagation();
	tinymce.EditorManager.execCommand('mceAddEditor',true, 'w-wysiwyg');
});