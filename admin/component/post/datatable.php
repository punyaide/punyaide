...
if($currentRoleAccess->read_access == "Y"){
    ...
    while ( $aRow = mysql_fetch_array( $rResult ) )
    {
        ...
        for ( $i=1 ; $i<count($aColumns) ; $i++ )
        {
			...
			$strlink = preg_replace("/\/admin\/component\/post\/(datatable\.php$)/","",$str);
			$valid = $aRow['id_category'];
			$tablecat = new WisasiTable('category');
			...
			$row[] = "$aRow[title]<br />
					...
					<div class='btn-group btn-group-xs pull-right'>
						...
						<a href='component/post/facebook.php?id=$aRow[id_post]' class='btn btn-xs btn-default'><i class='fa fa-facebook text-info'></i> Share</a>
						<a href='component/post/twitter.php?id=$aRow[id_post]' class='btn btn-xs btn-default'><i class='fa fa-twitter text-primary'></i> Share</a>
