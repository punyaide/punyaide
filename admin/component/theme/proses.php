...
if ($mod=='theme' AND $act=='delete'){
	if($currentRoleAccess->delete_access == "Y"){
		...
		$tabledel = new WisasiTable('theme');
	...
	if($currentRoleAccess->write_access == "Y"){
		...
		if (!empty($tmpName)){
			if (in_array($ekstensi, $extensionList)){
				...
				$destination_dir = "../../../content/$folder";
				if (file_exists($destination_dir)){
					unlink("../../../content/upload/$nama_file_unik");
					header('location:../../404.php');
				}else{
					$file = "../../../content/upload/$nama_file_unik";
...
elseif ($mod=='theme' AND $act=='edit'){
	if($currentRoleAccess->modify_access == "Y"){
		...
		$filename = "../../../content/$folder/$valid";
		if (file_exists("$filename")){
			$data = $_POST['code_content'];
			$data = str_replace("textareawisasicms", "textarea", $data);
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:70px; background-color:#f0f0f0;">
<link rel="stylesheet" href="<?=$website_url;?>/content/<?=$folder;?>/..." type="text/css" media="all" />
</textarea></p>';
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:110px; background-color:#f0f0f0;">
<?=$website_url;?>
<?=$website_name;?>
<?=$meta_description;?>
<?=$folder;?>
</textarea></p>';
echo '<p>Code berikut digunakan untuk pemanggilan menu dinamis WisasiCMS.';
echo '<textarea class="form-control" id="whelper" style="width:100%; height:130px; background-color:#f0f0f0;">
<?php 
	$instance = new WisasiController;
	$menu = $instance->wisasi_menu(1, \'class="..."\', \'class="..."\');
?>
<?php echo $menu; ?>
</textarea></p>';
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:120px; background-color:#f0f0f0;">
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:230px; background-color:#f0f0f0;">
<?php
	$tablepopfoot = new WisasiTable(\'post\');
...
?>
		...
		Url img src : <?=$website_url;?>/content/upload/medium/medium_<?=$popfoot->picture;?>
		Title dan alt: <?=$popfoot->title;?>
		Tanggal : <?=tgl_indo($popfoot->date);?>
<?php } ?>
</textarea></p>';
echo '<p>Pemanggilan data tag teratas pada bagian footer.';
echo '<textarea class="form-control" id="whelper" style="width:100%; height:200px; background-color:#f0f0f0;">
<?php
	$tabletag = new WisasiTable(\'tag\');
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:200px; background-color:#f0f0f0;">
<?php
	$tablesubs = new WisasiTable(\'subscribe\');
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:70px; background-color:#f0f0f0;">
<script type="text/javascript" src="<?=$website_url;?>/content/<?=$folder;?>/..."></script>
</textarea></p>';
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:250px; background-color:#f0f0f0;">
<?php if ($member_register == "Y"){ if ($mod=="home"){ ?>
	<h3>Login Member</h3>
	<p>Login ke profilmu dan share segala sesuatu tentangmu sekarang.</p>
	<form name="login-form" method="post" action="<?=$website_url;?>/admin/login.php" autocomplete="off">
		<input type="text" name="username" id="username" placeholder="Username" />
		<input type="password" name="password" id="password" placeholder="Password" />
		Belum punya akun ? Klik <a href="<?=$website_url;?>/register" title="Register Member">di sini!</a>
		Lupa password ? Klik <a href="<?=$website_url;?>/admin" title="Lupa password">di sini!</a>
		<input type="submit" name="submit" value="Login" />
	</form>
<?php }} ?>
</textarea></p>';
echo '<p>Pemanggilan data post teratas pada bagian sidebar.';
echo '<textarea class="form-control" id="whelper" style="width:100%; height:260px; background-color:#f0f0f0;">
<?php
	$tablepop = new WisasiTable(\'post\');
	$pops = $tablepop->findAllLimitBy(hits, active, \'Y\', DESC, \'15\');
	foreach($pops as $pop){
		$tablepopc = new WisasiTable(\'comment\');
		$totalpop = $tablepopc->numRowByAnd(id_post, $pop->id_post, active, \'Y\');
?>
		Url a href : <?php echo "$website_url/detailpost/$pop->seotitle"; ?>
		Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$pop->picture;?>
		Title dan alt : <?=$pop->title;?>
		Tanggal : <?=tgl_indo($pop->date);?>
<?php } ?>
</textarea></p>';
echo '<p>Pemanggilan data post terbaru pada bagian sidebar.';
echo '<textarea class="form-control" id="whelper" style="width:100%; height:340px; background-color:#f0f0f0;">
<?php
	$tablerec = new WisasiTable(\'post\');
	$recs = $tablerec->findAllLimitBy(id_post, active, \'Y\', DESC, \'5\');
	foreach($recs as $rec){
		$validrec = $rec->id_category;
		$tablecatrec = new WisasiTable(\'category\');
...
?>
		...
		Img src post : <?=$website_url;?>/content/upload/medium/medium_<?=$rec->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:320px; background-color:#f0f0f0;">
<?php
	$tablecom = new WisasiTable(\'comment\');
	$coms = $tablecom->findAllLimitBy(id_comment, active, \'Y\', DESC, \'5\');
	foreach($coms as $com){
		$validcom = $com->id_post;
		$tablecompo = new WisasiTable(\'post\');
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:150px; background-color:#f0f0f0;">
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:220px; background-color:#f0f0f0;">
<?php
	$tableslider = new WisasiTable(\'post\');
...
?>
		...
		Img src : <?=$website_url;?>/content/upload/<?=$slider->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:270px; background-color:#f0f0f0;">
<?php
	$tablecona = new WisasiTable(\'post\');
...
?>
		...
		Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$cona->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:270px; background-color:#f0f0f0;">
<?php
	$tableconb = new WisasiTable(\'post\');
...
?>
		...
		Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$conb->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:370px; background-color:#f0f0f0;">
<?php
	$tablegallery = new WisasiTable(\'gallery\');
	$gallerys = $tablegallery->findAllLimit(id_gallery, DESC, \'20\');
	foreach($gallerys as $gallery){
		$idalb = $gallery->id_album;
		$tablecalb = new WisasiTable(\'album\');
		$currentCalb = $tablecalb->findBy(id_album, $idalb);
		$currentCalb = $currentCalb->current();
		if ($currentCalb->active == \'Y\'){
?>
			Url a href : <?=$website_url;?>/content/upload/<?=$gallery->picture;?>
			Title dan alt gallery : <?=$gallery->title;?>
			Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$gallery->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:350px; background-color:#f0f0f0;">
<?php
...
?>
<?php if ($currentPag != "0"){ ?>
	Img src : <?=$website_url;?>/content/upload/<?=$currentPag->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:850px; background-color:#f0f0f0;">
<?php
	$title = $val->validasi($_GET[\'idc\'],\'xss\');
	$tabledcat = new WisasiTable(\'category\');
...
<?php if ($currentDcat != "0"){ ?>
	...
	?>
			...
			Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$dcpost->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:600px; background-color:#f0f0f0;">
<?php
	$title = $val->validasi($_GET[\'id\'],\'xss\');
	$detail = new WisasiTable();
	...
	if ($currentDetail > 0){
	$tabledpost = new WisasiTable(\'post\');
	...
	$batas = 5;
	$posisi = $p->cariPosisi($batas);
	$tabledcom = new WisasiTable(\'comment\');
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:430px; background-color:#f0f0f0;">
...
Img src : <?=$website_url;?>/content/upload/<?=$currentDetail->picture;?>
Isi post : <?=$contentdet;?>
List tag terkait :
	<?php
		$tabletag = new WisasiTable(\'tag\');
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:280px; background-color:#f0f0f0;">
...bersambung dari code di atas...
Profil Penulis
<?php
	$filename = "$website_url/content/upload/user-$currentDetail->id_user.jpg";
	if (file_exists("$filename")){
		echo "<img src=\'$website_url/content/upload/user-$currentDetail->id_user.jpg\' alt=\'$currentDetail->nama_lengkap\' />";
	}else{
		echo "<img src=\'$website_url/content/upload/user-editor.jpg\' alt=\'$currentDetail->nama_lengkap\' />";
	}
?>
...
Artikel Terkait
<?php
	$tablerelated = new WisasiTable(\'post\');
...
?>
		...
		Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$tablerelated->picture;?>
		Isi highlight post : <?=cuthighlight(\'title\', $tablerelated->title, \'20\');?>
<?php } ?>
...bersambung ke code di bawah ini...
</textarea></p>';
echo '<p>Code untuk menampilkan list komentar.';
echo '<textarea class="form-control" id="whelper" style="width:100%; height:560px; background-color:#f0f0f0;">
...bersambung dari code di atas...
...
?>
			Img avatar src : <?=$website_url;?>/content/chingsy/images/avatar.jpg
			Nama : <?=$compost->name;?>
			<?php if ($compost->url != \'\'){ ?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:290px; background-color:#f0f0f0;">
...bersambung dari code di atas...
Tinggalkan Komentar
<form action="<?=$website_url;?>/postcom.php" method="post">
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:590px; background-color:#f0f0f0;">
<?php
	$p = new Paging;
	$batas = 6;
	$posisi = $p->cariPosisi($batas);
	$tablegal = new WisasiTable(\'gallery\');
...
?>
			...
			Title dan alt gallery : <?=$gallery->title;?>
			Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$gallery->picture;?>
			Img src large : <?=$website_url;?>/content/upload/<?=$gallery->picture;?>
			Title dan alt album : <?=$currentCalb->title;?>
<?php
		}
	}
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:980px; background-color:#f0f0f0;">
<?php
if ($_GET[\'search\'] == ""){
	...
	$batas = 5;
	$posisi = $p->cariPosisi($batas);
	$tablesearch = new WisasiTable(\'post\');
...
?>
		<?php
			foreach($searchposts as $searchpost){
				$tabledscom = new WisasiTable(\'comment\');
				$totaldscom = $tabledscom->numRowByAnd(id_post, $searchpost->id_post, active, \'Y\');
				$tablecatds = new WisasiTable(\'category\');
		...
		?>
				...
				Img src : <?=$website_url;?>/content/upload/medium/medium_<?=$searchpost->picture;?>
...
echo '<textarea class="form-control" id="whelper" style="width:100%; height:240px; background-color:#f0f0f0;">
<h1>Register Member</h1>
<p>Daftarkan profilmu dan share segala sesuatu tentangmu sekarang.</p>
<form name="register-form" method="post" action="<?=$website_url;?>/admin/actregister.php" autocomplete="off">
	<input type="text" name="username" id="username" placeholder="Username" />
