<?php
if (!file_exists("library/configure.php")){
	$now = gmdate('D, d M Y H:i:s') . ' GMT';
	header("Expires: $now");
	header("Last-Modified: $now");
	header("Cache-Control: no-cache, must-revalidate");
	header("Pragma: no-cache");
	echo "Maaf, WisasiCMS anda <b>belum terinstall</b>\n";
	...
	include_once 'admin/component/menumanager/includes/configure.php';
	...
	$tablestat = new WisasiTable('traffic');
	$totalstat = $tablestat->numRowByAnd(ip, $ipstat, tanggal, $tanggalstat);

	if($totalstat == 0){
		$tablestatp = new WisasiTable('traffic');
		...
		$tablestatp2 = new WisasiTable('traffic');
		...
		$tablestat2 = new WisasiTable('traffic');
